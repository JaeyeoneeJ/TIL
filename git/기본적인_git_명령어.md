...
### 3-1) 원격 저장소에 있는 브랜치 삭제
- 삭제한 브랜치는 로컬 저장소에만 삭제되며, 원격 저장소에는 삭제된 브랜치가 남아있을 수 있음
- 원격 저장소에서도 브랜치를 삭제하려면 `git push` 명령을 사용해야 함
- 예를 들어, `origin` 원격 저장소에서 `old-branch` 브랜치를 삭제하려면 다음과 같이 명령어를 실행함
```bash
git push origin --delete old-branch
```
- 해당 명령어를 실행하면 로컬 및 원격 저장소에서 해당 브랜치가 완전히 삭제됨
